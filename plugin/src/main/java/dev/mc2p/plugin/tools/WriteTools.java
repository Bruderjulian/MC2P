    ...
    public static void register(ToolRegistry registry, ServerFacade facade, BackendConfig config) {
        ...
        registry.register(new ToolSpec(
                ...
                (args, auth) -> {
                    ...
                    String worldKey = Args.string(args, "world");
                    if (worldKey == null || !Validators.isSafeWorldKey(worldKey) || !facade.worldExists(worldKey)) {
                        throw new ToolException("world is required and must exist");
                    }
                    if (!auth.restrictions().isWorldAllowed(worldKey)) {
                        throw new ToolException("world '" + worldKey + "' is not allowed for this token");
                    }
